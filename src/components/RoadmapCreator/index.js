...
const RoadmapCreatorView = () => {
    ...
    useEffect(() => {
        d3.select(viewContainer.current)
            .call((viewContainer) => updateView(viewContainer, { nodes, links }))
    }, [nodes, links])
...
const updateView = (viewContainer, { nodes, links }) => {
    ...
    const circle1 = (viewContainer).append("svg").attr("viewBox", "-15 15 15 15").style("position", "absolute").attr("transform", "translate(100)").append("circle").attr("r", 20)
    const circle2 =(viewContainer).append("svg").style("position", "absolute").attr("transform", "translate(75)").append("circle").attr("r", 30)
    const editable = viewContainer.append("div").style("position", "absolute").attr("contenteditable", true).text("akdlfj")
}
