...
export function onTransitionNodeClick(dClicked, iClicked, list, DOMEle, app) {
  ...
  app.optionG.append("foreignObject").lower().attrs({
    width: 175,
    height: 60,
    x: globalRadius,
  });
  ...
  function openForm(d, i) {
    ...
    app.transitionForm
      ...
      .on("end", function () {
        ...
        if (!app.showModal) d3.select(this).node().focus();
      });
  ...
  paragraph
    ...
    .on("keydown", function () {
      ...
      if (
        ...
        d3.event.preventDefault();
        app.setMessage(
          "Please write something before starting a new line",
          2000
        );
        return;
      } else {
    ...
    .on("keyup", function () {
      if (d3.event.keyCode === 13 && !d3.event.shiftKey) {
        ...
        app.setMessage(
          "This textbox has reached the 2000 character limit",
          1000
        );
