...
import { nodes as initialNodes, links as initialLinks } from "./data.js";

class RoadmapCreatorOld extends Component {
  constructor(props) {
    super(props);
    this.state = {
      showManual: false,
  ...
  setMessage(error, waitTime) {
    ...
    this.setState(
      ...
      () => {
        ...
        div.style("opacity", 0).transition().duration(750).style("opacity", 1);
  ...
  setUp() {
    ...
    this.container = this.svg
      ...
      .attr(
        "transform",
        this.previousTransform ? this.previousTransform : null
      );
    // set up dragLine
  ...
  tick() {
    ...
    if (
      ...
      this.optionGroup.attr("transform", function (d) {
        ...
        },${app.selectedNode.y + app.selectedNode.height / 2 - 25})`;
      });
    }
    ...
    this.path.attr("d", (d) => {
      ...
      var string = `M${d.source.x + d.source.width / 2},${
  ...
  restart() {
    ...
    if (this.nodes.length === 0) {
      this.container
        ...
        .text("Click this to add Dummy Data, else Ctrl Click")
        .on(
          "click",

          () => {
            app.nodes = initialNodes;
            app.links = initialLinks;
            app.restart();
          }
        );
    }
    ...
    // UPDATE
    this.path = this.path.enter().append("svg:path").merge(this.path);
    ...
    textContainers
      .each(function (d, i) {
        ...
        var eachTextHeight = d3.select(this).select("text").node().getBBox()
          .height;
        var textGroup = d3.select(this).selectAll("text");
        var widthArray = [],
          heightArray = [];
        textGroup.each(function () {
          heightArray.push(d3.select(this).node().getBBox().height);
          widthArray.push(d3.select(this).node().getBBox().width);
        });
      ...
      .style("transform", function (d) {
        var bboxWidth = d3.select(this).node().getBBox().width;

        var bboxHeight = d3.select(this).node().getBBox().height;
        var toShiftX = 25;
    ...
    if (previousPreview === false) {
      ...
      if (this.state.shouldViewText) {
        this.setState({ shouldViewText: false }, () => {
          d3.selectAll("g.circleGroup").selectAll("foreignObject").remove();
        });
      }
  ...
  toggleManual = () => {
    this.setState((prevState) => {
      return { showManual: !prevState.showManual };
    });
  };
  onLoadIt = (
    app,
    ...
    this.forceUpdate();
    this.setMessage(
      "This would've loaded website content into node, but I disabled the cloud function responsible for that due to cost",
      3000
    );
...
export default RoadmapCreatorOld;
